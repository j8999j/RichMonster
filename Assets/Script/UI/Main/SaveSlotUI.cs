...
public class SaveSlotUI : MonoBehaviour
{
    ...
    private void CreateSlotUI(SaveSlotData slotData, int slotIndex)
    {
        ...
        // 刪除按鈕 (prefab 第 4 個子物件 / child index 3)
        var deleteButton = slotObj.transform.childCount > 3
            ? slotObj.transform.GetChild(3).GetComponent<Button>()
            : null;
        if (deleteButton != null)
        {
            deleteButton.gameObject.SetActive(!slotData.IsEmpty);
            if (!slotData.IsEmpty)
            {
                int capturedSlot = slotIndex;
                deleteButton.onClick.RemoveAllListeners();
                deleteButton.onClick.AddListener(() => ShowDeleteConfirm(capturedSlot));
            }
        }
    }
    ...
    private void ShowDeleteConfirm(int slotIndex)
    {
        ...
        if (deleteConfirmMessageText != null)
            deleteConfirmMessageText.text = $"確定要<color=red>刪除存檔 {slotIndex + 1}</color>嗎？\n刪除後無法復原。";
    ...
    {
        int slotIndex = _pendingDeleteSlotIndex;
        ...
        if (slotIndex >= 0)
            OnSlotDeleteRequested?.Invoke(slotIndex);
    }
