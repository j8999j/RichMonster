...
public class MonsterTradeView : MonoBehaviour
{
    ...
    [Header("交易組件")]
    [SerializeField, Tooltip("交易組件根物件")] 
    ...
    /// <summary>
    /// 根據交易滿意度呈現對應視覺 (目前暫時留空)
    /// </summary>
    public void ShowSatisfactionVisual(TradeSatisfaction satisfaction)
    {
        switch (satisfaction)
        {
            case TradeSatisfaction.Hated:
                // 厭惡
                break;
            case TradeSatisfaction.Okay:
                // 尚可
                break;
            case TradeSatisfaction.Satisfied:
                // 滿意
                break;
            case TradeSatisfaction.VerySatisfied:
                // 非常滿意
                break;
        }
    }
