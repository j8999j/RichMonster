...
{
    //summary>
    // 家具店商店類別，處理抽籤、價格計算與購買狀態同步等邏輯。
    // 含交易邏輯
    /// </summary>
    ///
    public class FurnituresShop : ShopBase, Itrade
    {
        void OnEnable()
        {
        ...
        {
            if (GameManager.Instance.IsPlayerMoveLocked(PlayerLockSources.FurnituresShop))
            {
                _shopUIView.SetVisible();
                GameManager.Instance.UnlockPlayerMove(PlayerLockSources.FurnituresShop);
                return;
            }
            ...
            _shopUIView.SetVisible();
            GameManager.Instance.LockPlayerMove(PlayerLockSources.FurnituresShop);
        }
