...
{
    //summary>
    // 食物店商店類別，處理抽籤、價格計算與購買狀態同步等邏輯。
    // 含交易邏輯
    /// </summary>
    ///
    public class FoodShop : ShopBase, Itrade
    {
        void OnEnable()
        {
        ...
        {
            if (GameManager.Instance.IsPlayerMoveLocked(PlayerLockSources.FoodShop))
            {
                _shopUIView.SetVisible();
                GameManager.Instance.UnlockPlayerMove(PlayerLockSources.FoodShop);
                return;
            }
            ...
            _shopUIView.SetVisible();
            GameManager.Instance.LockPlayerMove(PlayerLockSources.FoodShop);
        }
