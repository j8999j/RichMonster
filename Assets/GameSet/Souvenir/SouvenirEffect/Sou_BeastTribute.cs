...
namespace Souvenir
{
    /// <summary>
    /// 特殊紀念品：獸族敬意
    /// 條件：交易使獸族客人滿意（Satisfied 或 VerySatisfied）累計達到 50 次
    /// 進度跨局儲存，使用 IAchievementSave 機制寫入 illustrated_book.json
    /// </summary>
    [SouvenirDefinition("Sou_BeastTribute")]
    public class Sou_BeastTribute : SpecialSouvenir, IMonsterTradeWithRaceListener, ISpecialSouvenirSave, ISouvenirInteractive
    {
        public override string SouvenirID => "Sou_BeastTribute";

        // --- ISpecialSouvenirSave 欄位 (跨局存檔用) ---
        public override bool IsCompleted { get => IsCollected; set { } }

        // --- ISpecialSouvenirDisplayInfo 欄位 ---
        public override string DisplayName => "獸族的餽贈";
        public override string DisplayCondition => "累計讓獸族客人滿意 50 次";
        public override string DisplayDescription => "來自獸族部落的感謝，見證了你的交易手腕。";
        public override string ProgressText => $"{SatisfiedCount}/{RequiredCount}";
        public override float ProgressRatio => Mathf.Clamp01((float)SatisfiedCount / RequiredCount);

        /// <summary> 累計使獸族客人滿意的次數 </summary>
        public int SatisfiedCount { get; set; }

        private const int RequiredCount = 50;
        private const string TargetRace = "獸族";
        ...
        public Sou_BeastTribute() : base()
        {
            EffectName = "野獸的足跡將會引導你";
        }

        /// <summary> 從存檔中恢復進度 </summary>
        public void Initialize()
        {
            var saved = DataManager.Instance.GetSpecialSouvenirSaveData(SouvenirID) as Sou_BeastTribute;
            if (saved != null)
            {
